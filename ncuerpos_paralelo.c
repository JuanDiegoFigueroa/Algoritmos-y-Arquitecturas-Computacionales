/*  ncuerpos calcula la trayectoria de n cuerpos debido a la atracción gravitacional entre ellos. En base a su masa y sus posiciones y velocidades inciales.
  Compilar: 
    $ gcc -fopenmp ncuerpos_paralelo.c -o ncuerpos_p -lm -std=c99
  Ejecutar:
    $ ./ncuerpos_p
  Estudiantes:
...
double* siguiente_valor(double x[], double y[], double vx[], double vy[], int n, double h, double m[]){
  ...
  double array2D[4*n];      // Se guardan los resultados
...
int main(){
   ...
   int N=32000;                    // Cantidad de pasos. Aumentar para añadir tiempo de animación.
   double h=5000;                  // Paso de tiempo. Modificar según se necesite.
